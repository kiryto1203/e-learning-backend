...
public interface IQuestionBankRepository extends JpaRepository<QuestionBank, String> {
    ...
    List<QuestionBank> fetchSiblings(String questionParentCode);
    @Query(value = "select * from question_bank where question_type != ?2 and subcategory_code = ?1 order by RAND() limit 1", nativeQuery = true)
    QuestionBank fetchOneRandomQuestionBySubCategoryCodeAndNotQuestionType(String subCategoryCode, int questionType);
